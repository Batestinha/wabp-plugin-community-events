...
Standalone WABS package `official.community-events` version `0.15.0`, requiring WABP core API `^0.3.0`. It retains the plugin ID, account-owned `events` database, data version 18, all SQL migrations, scoped configuration, event identifiers, creator bindings, subgroup checkpoints and receipt formats.
