...
def build_release():
    ...
    for contract in contracts['contracts']:
        for base in [root / 'contracts', root / 'src/contracts']:
            pristine = (base / contract['upstreamPath']).read_bytes()
            if hashlib.sha256(pristine).hexdigest() != contract['upstreamSha256']:
                raise ValueError('Pristine upstream contract identity changed')
            rebuilt = pristine.decode('utf-8')
            for patch in contract['patches']:
                if patch['operation'] != 'replace-import' or patch['before'] not in rebuilt:
                    raise ValueError('Vendored import patch conflicts with upstream')
                if not patch['before'].startswith("'../") or 'packages/plugin-sdk/src/' not in patch['before'] or not patch['after'].startswith("'@wabs/plugin-sdk/"):
                    raise ValueError('Undeclared vendored patch operation')
                rebuilt = rebuilt.replace(patch['before'], patch['after'])
            data = (base / contract['vendoredPath']).read_bytes()
            if rebuilt.encode('utf-8') != data or hashlib.sha256(data).hexdigest() != contract['vendoredSha256']:
                raise ValueError('Vendored source cannot be reconstructed from recorded upstream and patches')
    cache = root / '.cache'
